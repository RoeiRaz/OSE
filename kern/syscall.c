...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	user_mem_assert(e, tf, sizeof(*tf), 0);
	
	// Set the environment trapframe
	e->env_tf = *tf;
	
	// Turn on interrupt flag
	e->env_tf.tf_eflags |= FL_IF;
	
	// Modify cpl to prevent an environment from becoming the kernel
	e->env_tf.tf_cs |= 3; // like doing 'and 0b000...00011, %cs' (pseudo asm)
